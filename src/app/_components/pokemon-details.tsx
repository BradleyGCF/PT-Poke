...
export function PokemonDetails({ pokemon }: PokemonDetailsProps) {
  ...
  const getCustomBackground = (type: string): string => {
    const backgrounds: Record<string, string> = {
      ...
      grass:
        "linear-gradient(135deg, #d1fae5 0%, #a7f3d0 30%, #6ee7b7 60%, #34d399 100%)",
      ice: "linear-gradient(135deg, #f0f9ff 0%, #e0f2fe 30%, #bae6fd 60%, #7dd3fc 100%)",
      fighting:
        "linear-gradient(135deg, #fce7f3 0%, #fbcfe8 30%, #f9a8d4 60%, #ec4899 100%)",
      poison:
        "linear-gradient(135deg, #ede9fe 0%, #ddd6fe 30%, #c4b5fd 60%, #a78bfa 100%)",
      ground:
        "linear-gradient(135deg, #fef7cd 0%, #fef08a 30%, #fde047 60%, #eab308 100%)",
      flying:
        "linear-gradient(135deg, #f1f5f9 0%, #e2e8f0 30%, #cbd5e1 60%, #94a3b8 100%)",
      psychic:
        "linear-gradient(135deg, #fdf2f8 0%, #fce7f3 30%, #fbcfe8 60%, #f9a8d4 100%)",
      ...
      ghost:
        "linear-gradient(135deg, #f8fafc 0%, #e2e8f0 30%, #cbd5e1 60%, #64748b 100%)",
      dragon:
        "linear-gradient(135deg, #f3e8ff 0%, #e9d5ff 30%, #d8b4fe 60%, #c084fc 100%)",
      ...
      fairy:
        "linear-gradient(135deg, #fdf2f8 0%, #fce7f3 30%, #fbcfe8 60%, #f472b6 100%)",
    };
    ...
    <div className="relative min-h-screen">
      ...
      <div className="relative z-10 space-y-6 p-6">
        ...
        <div className="grid grid-cols-1 items-start gap-8 lg:grid-cols-2">
          ...
          <div className="sticky top-4 flex items-start justify-center self-start">
            <div className="h-[450px] w-full max-w-[450px] lg:h-[550px] lg:w-[550px]">
              {pokemon.sprite ? (
                <SafeImage
                  src={pokemon.sprite}
                  alt={pokemon.name}
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-wrap gap-3">
              {pokemon.types.map((type) => (
                <span
                  key={type}
                  className={`rounded-full px-4 py-2 font-medium text-white capitalize ${getTypeColor(type)}`}
                >
                  {type}
                </span>
              ))}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="rounded-lg bg-gray-50 p-4">
                  <div className="mb-1 text-sm text-gray-500">Height</div>
                  <div className="text-lg font-semibold">
                    {pokemon.height / 10} m
                  </div>
                </div>
                <div className="rounded-lg bg-gray-50 p-4">
                  <div className="mb-1 text-sm text-gray-500">Weight</div>
                  <div className="text-lg font-semibold">
                  ...
                  <Link
                    ...
                    className={`group block ${
                      evolution.name === pokemon.name
                        ? "rounded-lg ring-4 ring-red-400"
                        : "rounded-lg transition-all hover:ring-2 hover:ring-gray-300"
                    }`}
                  >
                    <div className="rounded-lg border-2 border-gray-100 bg-white p-4 shadow-sm transition-colors group-hover:border-gray-200">
