...
async function getCachedPokemonDetails(pokemon: {
  name: string;
  url: string;
}): Promise<PokemonListItemWithDetails | null> {
...
export async function getEvolutionChain(speciesUrl: string): Promise<string[]> {
  try {
    ...
    if (
      ...
      !("evolution_chain" in speciesData)
    ) {
      return [];
    }
...
export async function getDetailedEvolutions(
  speciesUrl: string,
  ...
  try {
    ...
    for (const name of evolutionNames) {
      try {
        ...
        const parsedPokemon = PokemonSchema.parse(pokemonData);
        evolutions.push(
          EvolutionItemSchema.parse({
            id: parsedPokemon.id,
            name: parsedPokemon.name,
            sprite:
              parsedPokemon.sprites.other?.["official-artwork"]
...
export async function fetchPokemonDetails(
  nameOrUrl: string,
): Promise<BasicPokemonDetails> {
  const url = nameOrUrl.startsWith("http")
    ? nameOrUrl
    : `${POKEAPI_BASE_URL}/pokemon/${nameOrUrl}`;
...
export async function getListWithDetailsService(input: {
  ...
  try {
    ...
    const isOtherFiltered =
      (input.typeFilter && input.typeFilter.trim() !== "") ??
    ...
    if (isNameSearch) {
      ...
      for (let i = 0; i < quickMatches.length; i += batchProcessSize) {
        ...
        const validResults = batchResults.filter(
          (result): result is PokemonListItemWithDetails => result !== null,
        );
    ...
    if (input.typeFilter && input.typeFilter.trim() !== "") {
      filteredPokemon = filteredPokemon.filter((pokemon) =>
        pokemon.types.some(
          (type) => type.toLowerCase() === input.typeFilter!.toLowerCase(),
        ),
      );
    }
    if (input.generationFilter && input.generationFilter.trim() !== "") {
      ...
      filteredPokemon = filteredPokemon.filter(
        (pokemon) => pokemon.generation === input.generationFilter,
      );
    ...
    if (input.nameSearch) {
        ...
        evolutionPromises.push(
          (async () => {
            try {
              ...
              const evolutionNames = await getEvolutionChain(
      ...
      const evolutionNamesToFetch = Array.from(evolutionFamilies).filter(
        (name) => !searchResults.some((p) => p.name === name),
      );
        ...
        const evolutionResults = await Promise.all(
          batch.map(async (pokemonName) => {
            try {
              ...
              const speciesResponse = await fetch(parsedPokemon.species.url);
              if (!speciesResponse.ok)
                throw new Error(`Failed to fetch species for ${pokemonName}`);
              const speciesData: unknown = await speciesResponse.json();
              const parsedSpecies = PokemonSpeciesSchema.parse(speciesData);
              ...
              return PokemonListItemWithDetailsSchema.parse({
                ...
                name: parsedPokemon.name,
                types: parsedPokemon.types.map((t) => t.type.name),
                generation: getGenerationDisplayName(
                  parsedSpecies.generation.name,
                ),
                sprite:
                  parsedPokemon.sprites.other?.["official-artwork"]
                    ?.front_default ?? parsedPokemon.sprites.front_default,
              ...
              logger.error(
                `Error fetching evolution details for ${pokemonName}`,
                { error },
              );
              return null;
            }
    ...
    const totalCount = isFiltered
      ? Math.max(
          ...
          input.offset +
            ...
            (paginatedResults.length === input.limit ? input.limit : 0),
        )
      : parsedListData.count;
    const hasNext = isFiltered
      ? filteredPokemon.length > endIndex ||
        (paginatedResults.length === input.limit &&
          filteredPokemon.length === batchSize)
      : !!parsedListData.next;
